	...
	public GameScreen(LetsRace letsRace) {
		if (!debug) {
			...
			renderer = new FRGameRenderer(gameRef.myPlayerNo, gameWorldRef,
					gameRef.stage.getBatch());
			adapter = new FRNetworkInputHandler(gameRef.googleServices,
					gameRef.client.serverID);
			...
			adapter = new FRCarKeyboardInputHandler(
					gameWorldRef.carHandler.cars.get(0));
			Gdx.input.setInputProcessor(adapter);
		}
	}
