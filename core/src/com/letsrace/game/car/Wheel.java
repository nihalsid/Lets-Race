...
public class Wheel {
	...
	public Wheel(World world, Car car, float scale,
		...
		if (this.revolving) {
			...
			jointdef.enableMotor = false; // we'll be controlling the wheel's
											// angle manually
			world.createJoint(jointdef);
		} else {
			PrismaticJointDef jointdef = new PrismaticJointDef();
			jointdef.initialize(this.car.body, this.body,
					this.body.getWorldCenter(), new Vector2(1, 0));
	...
	public void setAngle(float angle) {
		 ...
		 */
		this.body.setTransform(body.getPosition(), this.car.body.getAngle()
	...
	public Vector2 getLocalVelocity() {
		 ...
		 */
		return this.car.body.getLocalVector(this.car.body
				.getLinearVelocityFromLocalPoint(this.body.getPosition()));
	};

	public Vector2 getDirectionVector() {
		/*
		 * returns a world unit vector pointing in the direction this wheel is
		 * moving
		 */
		...
		return directionVector.rotate((float) Math.toDegrees(this.body
				.getAngle()));
	};

	public Vector2 getKillVelocityVector() {
		/*
		 * substracts sideways velocity from this wheel's velocity vector and
		 * returns the remaining front-facing velocity vector
		 */
